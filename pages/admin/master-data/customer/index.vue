  ...
  <section class="space-y-5">
    <div class="rounded-md border border-slate-200 bg-white">
      <div class="flex flex-col gap-3 border-b border-slate-200 p-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 class="text-base font-semibold text-slate-900">{{ t('customer.title') }}</h2>
          <p class="mt-1 text-sm text-slate-500">{{ t('customer.description') }}</p>
        </div>
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <el-button v-if="can('customer-create')" type="primary" @click="openCreateDialog">
      ...
      >
        ...
        <el-table-column :label="t('columns.gender')" min-width="130">
          <template #default="{ row }">{{ displayGender(row.gender) }}</template>
        </el-table-column>
        <el-table-column :label="t('columns.dob')" min-width="150">
          <template #default="{ row }">{{ displayDate(row.dob) }}</template>
        </el-table-column>
        ...
        <el-table-column :label="t('columns.action')" fixed="right" width="140" align="center">
          <template #default="{ row }">
            <div class="flex items-center justify-center gap-2">
              <el-tooltip v-if="can('customer-edit')" :content="t('edit')" placement="top">
                <el-button type="success" circle @click="openEditDialog(row)">
                  <Icon name="solar:pen-2-outline" size="17" />
                </el-button>
              </el-tooltip>
              <el-tooltip v-if="can('customer-delete')" :content="t('delete')" placement="top">
    ...
    <el-dialog
      ...
      :title="editingItem ? t('customer.edit_title') : t('customer.create_title')"
      width="760px"
      align-center
      append-to-body
    ...
    >
      ...
      >
        <el-tabs v-model="activeFormTab" type="border-card">
          <el-tab-pane :label="t('customer.general_tab')" name="general">
            <div class="grid gap-4 sm:grid-cols-2">
              <el-form-item :label="t('columns.code')" prop="code">
                <el-input v-model="form.code" placeholder="CUS-001" />
              </el-form-item>
              <el-form-item :label="t('columns.gender')" prop="gender">
                <el-select
                  v-model="form.gender"
                  class="w-full"
                ...
                >
                  <el-option :label="t('customer.gender_male')" value="male" />
                  <el-option :label="t('customer.gender_female')" value="female" />
                </el-select>
              </el-form-item>
            </div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <el-form-item :label="t('columns.name_en')" prop="nameEn">
                <el-input v-model="form.nameEn" placeholder="John Doe" />
              </el-form-item>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <el-form-item :label="t('columns.dob')" prop="dob">
                <el-date-picker
                  v-model="form.dob"
                  type="date"
                  value-format="YYYY-MM-DD"
                  :placeholder="t('columns.dob')"
                  popper-class="customer-date-picker-popper"
                  class="!w-full"
                />
              </el-form-item>
...
const endpoint = 'admin/master-data/customers'
...
const rules = computed<FormRules>(() => ({
  code: [{ required: true, message: t('customer.code_required'), trigger: 'blur' }],
  nameEn: [{ required: true, message: t('customer.name_en_required'), trigger: 'blur' }],
  nameKh: [{ required: true, message: t('customer.name_kh_required'), trigger: 'blur' }],
  gender: [{ required: true, message: t('customer.gender_required'), trigger: 'change' }],
}))

watchEffect(() => {
  breadcrumbStore.setPageTitle(t('menu.customer'))
  breadcrumbStore.setBreadcrumbs([t('menu.master_data'), t('menu.customer')])
})
...
const displayDate = (value: Customer['dob']) => {
  if (!value) return '-'
...
const displayCreatedAt = (createdAt: Customer['createdAt']) => {
  if (!createdAt) return '-'
...
const displayGender = (gender: Customer['gender']) => {
  if (gender === 'male') return t('customer.gender_male')
  if (gender === 'female') return t('customer.gender_female')

  return '-'
}

const displayCreatedBy = (createdBy: Customer['createdBy']) => {
  if (!createdBy) return '-'
...
const loadProfilePreview = async (profile: ProfileValue) => {
  ...
  try {
    ...
    useNotification(getErrorMessage(error, t('customer.preview_failed')), 'error')
  }
}
...
const resetForm = (value: Partial<Customer> = emptyForm()) => {
  form.code = value.code ?? ''
  form.nameEn = value.nameEn ?? ''
  form.nameKh = value.nameKh ?? ''
  form.gender = value.gender ?? 'male'
  form.dob = value.dob ? String(value.dob).slice(0, 10) : ''
...
const loadItems = async () => {
  try {
    ...
    useNotification(getErrorMessage(error, t('customer.load_failed')), 'error')
  } finally {
    loading.value = false
  }
}
...
const submit = async () => {
  ...
  try {
    ...
    useMessage(editingItem.value ? t('customer.updated') : t('customer.created'))
    dialogVisible.value = false
    loadItems()
  } catch (error) {
    useNotification(getErrorMessage(error, t('customer.save_failed')), 'error')
  } finally {
    submitting.value = false
  }
}
...
const deleteItem = async (item: Customer) => {
  try {
    await ElMessageBox.confirm(
      t('customer.confirm_delete', { code: item.code }),
      t('customer.delete_title'),
      {
    ...
    useNotification(getErrorMessage(error, t('customer.delete_failed')), 'error')
  }
}
