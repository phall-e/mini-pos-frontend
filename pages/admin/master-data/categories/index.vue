    ...
    <div class="rounded-md border border-slate-200 bg-white">
      <div class="flex flex-col gap-3 border-b border-slate-200 p-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <el-button v-if="can('category-create')" type="primary" @click="openCreateDialog">
        ...
        <el-table-column :label="t('columns.action')" fixed="right" width="140" align="center">
          <template #default="{ row }">
            <div class="flex items-center justify-center gap-2">
              <el-tooltip v-if="can('category-edit')" :content="t('edit')" placement="top">
                <el-button type="success" circle @click="openEditDialog(row)">
                  <Icon name="solar:pen-2-outline" size="17" />
                </el-button>
              </el-tooltip>
              <el-tooltip v-if="can('category-delete')" :content="t('delete')" placement="top">
...
const submit = async () => {
  ...
  try {
    ...
    useMessage(editingItem.value ? t('category.updated') : t('category.created'))
